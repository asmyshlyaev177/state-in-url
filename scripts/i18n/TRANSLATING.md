...
- **A stale translation** — already in your language, but behind the English.
  ...
  ```bash
  pnpm i18n diff <locale>        # e.g. pnpm i18n diff zh-CN
  ```
...
| `packages/urlstate/**/README.md` | Per-module API docs, 11 of them, plus `Limits.md`. |
| `packages/example-nextjs16/src/app/i18n/copy/en.ts` | English demo-site copy. Never edit. |
| `packages/example-nextjs16/src/app/i18n/copy/<dir>.ts` | Your demo-site target. |

### The demo-site copy is TypeScript, not Markdown

`i18n/copy/<dir>.ts` is `en.ts` with its values translated. Directory-style
names, lowercase: `zh-cn`, `pt-br`, `ja`, `ko`, `ru`, `es`, `fr`, `vi`.

- **Replace string values only.** Never add, remove, rename or reorder a key.
  The file is typed against `types.ts`, so a dropped key fails the build.
- **Leave the `// i18n:meta` line alone** except `status=pending` →
  `status=translated`.
- Keep the JSDoc comments — they show how split keys reassemble. Several keys
  are fragments of one sentence that wraps an inline `<Word>` or `<Mono>` in
  the markup; the JSDoc marks the markup's contribution in brackets. If the
  split cannot work in your language's word order, put the whole sentence in
  the first fragment and leave the others empty rather than breaking grammar.
- `meta.*.title` is a `<title>`: under about 60 characters.
  `meta.*.description` is a search snippet: 150–160.
- Escaping is TypeScript's: `\'` inside a single-quoted string, or switch that
  value to double quotes.

Verify with `cd packages/example-nextjs16 && npx tsc --noEmit && pnpm build`.

`README.zh-CN.md` and `README.ko.md` are **stale translations**, not skeletons —
both are two commits behind and each is missing the whole "Using with AI coding
agents" section and the rewritten "Hire me" section. Patch them from the diff;
do not start over. The other six are skeletons.
...
pnpm i18n:check       # READMEs and API docs: structure, links, anchors, drift
pnpm i18n:copy:check  # demo-site copy modules: stamps and staleness
pnpm i18n status      # progress overview
```

Run `i18n:toc` **before** `i18n:check` — the check validates anchors, and the
TOC is what makes them resolve.
