...
import { ALL_LOCALES, localizedName, SOURCE_LOCALE, switcherLine } from './locales.mjs';
import { codeFingerprint, headings, links, Slugger, splitFences } from './markdown.mjs';
 ...
 */
export function stampDocument({ repoRoot, baseName, locale, file, sourceBlob, status }) {
 ...
 * attempted.
 */
export function validate({ sourceText, targetText, baseName, locale, currentBlob, linkPrefix }) {
  const problems = [];
  ...
  if (!header) {
    ...
    if (meta.status !== 'translated') add('pending', `status=${meta.status ?? 'unset'}`);
    const expected = switcherLine(baseName, locale);
    if (!header.includes(expected)) add('switcher', 'language switcher line is out of date');
  }

  if (targetText.includes(TODO_MARKER)) add('todo', `${TODO_MARKER} still present`);

  const src = splitFences(sourceText);
  const tgt = splitFences(targetText);
  if (tgt.unterminated !== null) add('fence-unterminated', `line ${tgt.unterminated}`);

  if (src.fences.length !== tgt.fences.length) {
    add('fence-count', `${src.fences.length} code blocks in English, ${tgt.fences.length} here`);
  } else {
  ...
  if (srcHeadings.length !== tgtHeadings.length) {
    add('heading-count', `${srcHeadings.length} headings in English, ${tgtHeadings.length} here`);
  } else {
    const shape = (hs) => hs.map((h) => h.depth).join(',');
    if (shape(srcHeadings) !== shape(tgtHeadings)) add('heading-shape', 'heading levels are in a different order');
  }
  ...
  const unprefix = (url) =>
    linkPrefix && url.startsWith(`/${linkPrefix}/`) ? url.slice(linkPrefix.length + 1) : url;
  ...
  // off first.
  const codes = ALL_LOCALES.map((l) => l.code).filter((c) => c !== SOURCE_LOCALE);
  ...
  if (realMissing.length) add('link-missing', `${realMissing.length} link(s) dropped, e.g. ${realMissing[0]}`);
  if (realAdded.length) add('link-added', `${realAdded.length} link(s) not in English, e.g. ${realAdded[0]}`);
  ...
  if (broken.length) {
    add('anchor-broken', `${broken.length} anchor(s) point at no heading: ${broken.slice(0, 4).join(', ')}`);
  }
...
export function targetsFor(repoRoot, sourceRelPath) {
  ...
  return ALL_LOCALES.filter((l) => l.code !== SOURCE_LOCALE).map((l) => {
    ...
    return { locale: l, rel, abs: path.join(repoRoot, rel), exists: existsSync(path.join(repoRoot, rel)) };
  });
}
