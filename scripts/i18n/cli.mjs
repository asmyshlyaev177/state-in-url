...
import { replaceToc, tableOfContents, TOC_END, TOC_START } from './markdown.mjs';
import {
  blobHash,
  parseDocument,
  skeletonBody,
...
function docsToRun() {
  ...
  const found = documents.filter((d) => d.source === want || path.basename(d.source) === want);
  if (found.length === 0) {
    ...
    for (const target of targetsFor(repoRoot, doc.source)) {
      if (target.exists) {
        ...
        stampDocument({
          ...
          file: target.abs,
          sourceBlob: parseDocument(readFileSync(target.abs, 'utf8')).meta?.['source-blob'] ?? sourceBlob,
        });
        continue;
      }
      writeFileSync(target.abs, `${TODO_MARKER}\n\n${skeletonBody(sourceText)}`, 'utf8');
      stampDocument({
        repoRoot,
        baseName: path.basename(doc.source),
        locale: target.locale.code,
    ...
    for (const locale of ALL_LOCALES) {
      const rel =
        locale.code === SOURCE_LOCALE
          ? doc.source
          : targetsFor(repoRoot, doc.source).find((t) => t.locale.code === locale.code).rel;
      const file = abs(rel);
      if (!existsSync(file)) continue;
      if (!readFileSync(file, 'utf8').includes(TOC_START)) {
        console.log(`${c.dim}·${c.off} ${rel} ${c.dim}(no ${TOC_START} marker)${c.off}`);
        continue;
      }
...
function caseCollisions() {
  ...
  for (const doc of documents) {
    ...
    const names = [doc.source, ...targetsFor(repoRoot, doc.source).map((t) => t.rel)];
...
function report({ failOnPending }) {
  ...
  for (const group of caseCollisions()) {
    console.log(`${c.red}case collision${c.off}: ${group.join(' vs ')} — cannot coexist on macOS or Windows`);
    problemCount += 1;
  }
  ...
  for (const doc of documents) {
    ...
    for (const target of targetsFor(repoRoot, doc.source)) {
      ...
      if (!target.exists) {
        console.log(`${label} ${c.red}missing${c.off} ${c.dim}${target.rel}${c.off}`);
        problemCount += 1;
        continue;
      }
      ...
      const pending = problems.filter((p) => p.kind === 'pending' || p.kind === 'todo');
      const hard = problems.filter((p) => p.kind !== 'pending' && p.kind !== 'todo');
      ...
      console.log(`${label} ${c.red}${hard.length} problem(s)${c.off}${pending.length ? ` ${c.yellow}(untranslated)${c.off}` : ''}`);
      for (const p of hard) console.log(`         ${c.red}·${c.off} ${p.kind}: ${p.detail}`);
    }
  }
...
function resolveTarget(localeArg) {
  ...
  if (!locale || locale.code === SOURCE_LOCALE) {
    console.error(`${c.red}Unknown locale "${localeArg}". One of: ${LOCALES.map((l) => l.code).join(', ')}${c.off}`);
    process.exit(2);
  }
  return locale;
}
...
function diff() {
  ...
  for (const doc of docsToRun()) {
    const target = targetsFor(repoRoot, doc.source).find((t) => t.locale.code === locale.code);
    if (!target.exists) {
      console.log(`${c.yellow}${target.rel} does not exist yet — run \`init\`.${c.off}`);
      continue;
    }
    const recorded = parseDocument(readFileSync(target.abs, 'utf8')).meta?.['source-blob'];
    ...
    if (recorded === current) {
      console.log(`${c.green}${target.rel} is up to date with ${doc.source}.${c.off}`);
      continue;
    }
    console.log(`${c.bold}${doc.source}: ${recorded.slice(0, 8)} → ${current.slice(0, 8)}${c.off}\n`);
    ...
    try {
      ...
      execFileSync(
        'git',
        ['diff', '--no-color', '--no-index', '--src-prefix=was/', '--dst-prefix=now/', scratch, abs(doc.source)],
...
function stamp() {
  ...
  for (const doc of docsToRun()) {
    ...
    console.log(`${c.green}✓${c.off} ${target.rel} marked translated against ${doc.source}`);
  }
}
...
switch (command) {
  ...
  case 'stamp':
    ...
    break;
  default:
    console.error(`Unknown command "${command}". See the header of ${fileURLToPath(import.meta.url)}.`);
    process.exit(2);
}
