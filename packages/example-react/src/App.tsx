import { useCommonState } from 'state-in-url'

function App() {
  return (
    <div className='p-8'>
      <h1>useCommonState hook for React</h1>
...
const FirstComponent = () => {
  const { state, setState } = useCommonState(STATE)
...
const Label = (props: React.DetailedHTMLProps<React.LabelHTMLAttributes<HTMLLabelElement>, HTMLLabelElement>) => {
  return <label className={`mb-2 block select-none hover:cursor-pointer ${props.className}`}>{props.children}</label>
}
